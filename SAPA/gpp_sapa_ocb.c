...
GPPLONG gpp_sapa_buffer2ocb(pGPP_SAPA_OCB p_ocb, const GPPUCHAR *buffer, GPPLONG *byte_pos, GPPLONG *bit_pos)
{
	...
	GPPUINT1 isys;
	for (isys = 0; isys < GPP_SAPA_MAX_CONS; isys++)      //add header to all the systems used in this epoch
	{
		GPPLONG rc;
		sys = isys;
		...
		if (p_ocb->header_block[sys]->end_of_obc_set == 1)
			break;
	}
...
static GPPLONG gpp_sapa_ocb_header2buffer(const pGPP_SAPA_OCB p_ocb, const SAPA_OCB_HANDLE *ocbHdl, GPPUINT1 *svlist, GPPUCHAR *buffer, GPPLONG *byte_pos, GPPLONG *bit_pos)
{
	...
	gn_add_ulong_to_buffer(buffer, byte_pos, bit_pos, 1, header->end_of_obc_set);									//SF010
	gn_add_ulong_to_buffer(buffer, byte_pos, bit_pos, 1, header->reserved);										//SF069
	gn_add_ulong_to_buffer(buffer, byte_pos, bit_pos, 1, header->yaw_flag);										//SF008
	gn_add_ulong_to_buffer(buffer, byte_pos, bit_pos, 1, header->sat_ref_datum);									//SF009
	gn_add_ulong_to_buffer(buffer, byte_pos, bit_pos, 2, header->ephemeris_type);									//SF017

	//select only satellites with valid corrections and according to message
	if(ocbHdl->prn_bits[sys]&GPP_SAPA_OCB_BITS_CLK)	sv_prn_bits|=p_ocb->clk_prn_bits[sys];
	if(ocbHdl->prn_bits[sys]&GPP_SAPA_OCB_BITS_ORB)	sv_prn_bits|=p_ocb->orb_prn_bits[sys];
	if(ocbHdl->prn_bits[sys]&GPP_SAPA_OCB_BITS_PB)	sv_prn_bits|=p_ocb->pb_prn_bits[sys];
	if(ocbHdl->prn_bits[sys]&GPP_SAPA_OCB_BITS_CB)	sv_prn_bits|=p_ocb->cb_prn_bits[sys];
...
static GPPLONG gpp_sapa_ocb_sv2buffer(const pGPP_SAPA_OCB p_ocb, const SAPA_OCB_HANDLE *ocbHdl, GPPUINT1 sat, GPPUCHAR *buffer, GPPLONG *byte_pos, GPPLONG *bit_pos)
{
	...
	if(ocbHdl->ocb_bits[sat]&GPP_SAPA_OCB_BITS_ORB) ocb_bits|=(sv->ocb_bits&SAPA_OCB_ORB_BITS);
	if(ocbHdl->ocb_bits[sat]&GPP_SAPA_OCB_BITS_CLK) ocb_bits|=(sv->ocb_bits&SAPA_OCB_CLK_BITS);
	if(ocbHdl->ocb_bits[sat]&GPP_SAPA_OCB_BITS_PB) 	ocb_bits|=(sv->ocb_bits&SAPA_OCB_BIAS_BITS);
	if(ocbHdl->ocb_bits[sat]&GPP_SAPA_OCB_BITS_CB) 	ocb_bits|=(sv->ocb_bits&SAPA_OCB_BIAS_BITS);
...
static GPPLONG gpp_sapa_ocb_orb2buffer(const pGPP_SAPA_OCB p_ocb, GPPUINT1 sys, GPPUINT1 sat, GPPUCHAR *buffer, GPPLONG *byte_pos, GPPLONG *bit_pos)
{
	...
	gn_add_ulong_to_buffer(buffer, byte_pos, bit_pos,14,orb->ld_orbit[0]);//SF020
	gn_add_ulong_to_buffer(buffer, byte_pos, bit_pos, 14, orb->ld_orbit[1]);
...
static GPPLONG gpp_sapa_ocb_buffer2orb(pGPP_SAPA_OCB p_ocb, GPPUINT1 sys, GPPUINT1 sat, const GPPUCHAR *buffer, GPPLONG *byte_pos, GPPLONG *bit_pos)
{
	...
	if (p_ocb->header_block[sys]->yaw_flag == GPP_SAPA_YAW_FLAG_PRESENT) {
		GPPUINT2 invalid = GPP_SAPA_OCB_SAT_YAW_INVALID;
		orb.yaw_bits = gpp_sapa_buffer2float(buffer, byte_pos, bit_pos, GPP_SAPA_OCB_SAT_YAW_INVALID, 6, GPP_SAPA_SAT_YAW_RES, &invalid, &orb.sat_yaw);//SF021
	}
...
static GPPLONG gpp_sapa_ocb_clk2buffer(const pGPP_SAPA_OCB p_ocb, GPPUINT1 sys, GPPUINT1 sat, GPPUCHAR *buffer, GPPLONG *byte_pos, GPPLONG *bit_pos)
{
	...
	gn_add_ulong_to_buffer(buffer, byte_pos, bit_pos,14, clk->lclk_correction);							//SF020
...
static GPPLONG gpp_sapa_ocb_buffer2clk(pGPP_SAPA_OCB p_ocb, GPPUINT1 sys, GPPUINT1 sat, const GPPUCHAR *buffer, GPPLONG *byte_pos, GPPLONG *bit_pos)
{
	...
	gpp_sapa_buffer2float(buffer, byte_pos, bit_pos, SAPA_SAT_MIN, 14, SAPA_RES_SV_CORRECTION, NULL, &clk.clk_correction);   //SF020
...
static GPPLONG gpp_sapa_ocb_bias2buffer(const pGPP_SAPA_OCB p_ocb, const SAPA_OCB_HANDLE *ocbHdl, GPPUINT1 sat, GPPUCHAR *buffer, GPPLONG *byte_pos, GPPLONG *bit_pos)
{
	...
	//Phase Bias
	if(ocbHdl->ocb_bits[sat]&GPP_SAPA_OCB_BITS_PB){//add phase bias corrections to buffer
	...
	//Code Bias
	if(ocbHdl->ocb_bits[sat]&GPP_SAPA_OCB_BITS_CB){//add phase bias corrections to buffer
...
static GPPLONG gpp_sapa_ocb_pb2buffer(const pGPP_SAPA_OCB p_ocb, GPPUINT1 sys, GPPUINT1 sat, GPPUINT1 sig, GPPUCHAR *buffer, GPPLONG *byte_pos, GPPLONG *bit_pos)
{
	...
	gn_add_ulong_to_buffer(buffer, byte_pos, bit_pos, 14, pb->lpb_correction);		//SF020
...
static GPPLONG gpp_sapa_ocb_buffer2pb(pGPP_SAPA_OCB p_ocb, GPPUINT1 sys, GPPUINT1 sat, GPPUINT1 sig, const GPPUCHAR *buffer, GPPLONG *byte_pos, GPPLONG *bit_pos)
{
	...
	gpp_sapa_buffer2float(buffer, byte_pos, bit_pos, SAPA_SAT_MIN, 14, SAPA_RES_PB_CORRECTION, NULL, &pb.pb_correction);       //SF020
...
static GPPLONG gpp_sapa_ocb_cb2buffer(const pGPP_SAPA_OCB p_ocb, GPPUINT1 sys, GPPUINT1 sat, GPPUINT1 sig, GPPUCHAR *buffer, GPPLONG *byte_pos, GPPLONG *bit_pos)
{
	...
	gn_add_ulong_to_buffer(buffer, byte_pos, bit_pos, 11, cb->lcb_correction);						//SF029
...
static GPPLONG gpp_sapa_ocb_buffer2cb(pGPP_SAPA_OCB p_ocb, GPPUINT1 sys, GPPUINT1 sat, GPPUINT1 sig, const GPPUCHAR *buffer, GPPLONG *byte_pos, GPPLONG *bit_pos)
{
	...
	gpp_sapa_buffer2float(buffer, byte_pos, bit_pos, SAPA_CB_MIN, 11, SAPA_RES_CB_CORRECTION, NULL, &cb.cb_correction); //SF029
